...
const HeaderDesktop: React.FC<HeaderDesktopProps> = ({ user }) => {
  ...
  // ✅ NUEVO: Solo el ROL USUARIO ve la sección "TODAS"
  const esRolUsuario = user?.rol === Rol.USUARIO;
      ...
      <div className="relative flex justify-center border-t border-gray-200 shadow-sm py-3 text-[0.7rem] sm:text-sm font-bold uppercase font-sans tracking-wide">
        ...
        {esSuperAdmin && (
          <>
            <Link
              to="/super-admin"
              className={`relative mx-3 px-4 py-1 group ${isActive("/super-admin") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
            >
              PANEL MAESTRO
            ...
            <Link
              ...
              className={`relative mx-3 px-4 py-1 group ${isActive("/super-admin?tab=DEPTOS") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
            >
              DEPARTAMENTO
            ...
            <Link
              ...
              className={`relative mx-3 px-4 py-1 group ${isActive("/super-admin?tab=USUARIOS") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
            >
              USUARIOS
            ...
            <Link
              ...
              className={`relative mx-3 px-4 py-1 group ${isActive("/super-admin?tab=LOGS") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
            >
              TERMINAL
        ...
        {!esSuperAdmin && (
          <>
            <Link
              to="/pendientes"
              className={`relative mx-3 sm:mx-6 px-4 py-1 group ${isActive("/pendientes") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
            >
              PENDIENTES
            ...
            {user && puedeVerAdminTareas() && (
              <Link
                to="/admin"
                className={`relative mx-3 px-4 py-1 group ${isActive("/admin") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
              >
                GESTIONAR TAREAS
            ...
            {user && puedeVerGestionUsuarios() && (
              <Link
                to="/usuarios"
                className={`relative mx-3 px-4 py-1 group ${isActive("/usuarios") ? "text-amber-800" : "text-gray-900 hover:text-amber-800"}`}
              >
                MI EQUIPO
        ...
        <div className="absolute right-4 sm:right-6 top-1/2 -translate-y-1/2">
          {user && (
            <button
              ...
              className="group flex items-center gap-2 px-3 py-2 rounded-full border border-transparent hover:bg-red-100 hover:border-red-200 text-red-600 hover:text-red-800 transition-all duration-300 active:scale-95 cursor-pointer"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5 opacity-70 group-hover:opacity-100 transition-opacity">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" />
              </svg>
              <span className="hidden sm:inline text-sm font-bold tracking-wide">Cerrar Sesión</span>
            </button>
          )}
        </div>
      </div>
