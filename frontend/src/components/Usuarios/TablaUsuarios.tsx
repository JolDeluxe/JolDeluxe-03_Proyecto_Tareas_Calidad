                  ...
                  <tr key={row.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-3 py-3 text-center w-[15%]">
                      <span className={`px-2 py-1 text-xs font-bold rounded-full 
                        ${row.rol === Rol.ADMIN ? "bg-red-100 text-red-700 border border-red-200" :
                          row.rol === Rol.ENCARGADO ? "bg-blue-100 text-blue-700 border border-blue-200" :
                    ...
                    {/* ✅ ACCIONES ESCRITORIO (Estilo Admin/Acciones.tsx) */}
                    <td className="px-4 py-3 text-center w-[20%]">
                      <div className="flex items-center justify-center gap-3 h-7">
                        ...
                        {/* 🗑️ Botón Activar/Desactivar */}
                        {currentUser?.id !== row.id && (
                          <button
                            onClick={() => toggleEstatus(row)}
                            title={row.estatus === "ACTIVO" ? "Desactivar usuario" : "Reactivar usuario"}
                            className={`w-7 h-7 flex items-center justify-center rounded-md border transition-all duration-200
                              ${row.estatus === "ACTIVO"
                                ? "border-red-400 text-red-600 hover:bg-red-600 hover:text-white" // Estilo Cancelar
                                : "border-green-400 text-green-700 hover:bg-green-100" // Estilo Validar
                              }`}
                          >
                            {row.estatus === "ACTIVO" ? (
                              // Icono Trash (o Cancelar estilo Admin)
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" className="w-5 h-5" fill="currentColor">
                                <path d="M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z" />
                              </svg>
                            ) : (
                              // Icono Check (o Validar estilo Admin)
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" className="w-5 h-5" fill="currentColor">
          ...
          <div className="grid lg:hidden grid-cols-1 md:grid-cols-2 gap-3 p-2 items-start">
            {usuariosFiltrados.map((row) => (
              <div key={row.id} className="border border-gray-300 shadow-sm p-4 rounded-md bg-white relative">
                ...
                <div className="text-xs text-gray-600 space-y-1">
                  ...
                  <p>
                    ...
                    <span className={`font-semibold 
                      ${row.rol === Rol.ADMIN ? "text-red-700" :
                        row.rol === Rol.ENCARGADO ? "text-blue-700" :
                          "text-green-700"}`}>
                      {row.rol}
                    </span>
                  </p>
                ...
                {/* ✅ ACCIONES MÓVIL (Estilo Admin Footer) */}
