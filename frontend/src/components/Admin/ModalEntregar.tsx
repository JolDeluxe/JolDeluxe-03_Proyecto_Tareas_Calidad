...
const ModalEntrega: React.FC<ModalEntregaProps> = ({
  ...
  // Estado para la lista de archivos (igual que en ModalNueva)
  const [archivos, setArchivos] = useState<File[]>([]);

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // --- Manejadores de Archivos (Lógica mejorada) ---
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      // Convertimos a array
      const nuevosArchivos = Array.from(e.target.files);

      // Validación simple de cantidad (máximo 5 en total)
      if (archivos.length + nuevosArchivos.length > 5) {
        alert("Solo puedes subir un máximo de 5 evidencias.");
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!comentario.trim()) {
      setError("El comentario de entrega es obligatorio.");
      return;
    }
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      setError(
        ...
        "Error al enviar a revisión. Intente de nuevo."
      );
    } finally {
      setLoading(false);
    }
        ...
        >
          ...
          <div className="flex-grow overflow-y-auto p-6 text-gray-800">
            <div className="flex flex-col gap-5">
              ...
              {/* --- ZONA DE ARCHIVOS (Diseño ModalNueva) --- */}
              <div>
                ...
                <input
                  ...
                  className="hidden" // 👈 Clave para ocultar el input feo
                  disabled={loading}
                />
                  ...
                  <div className="mt-3">
                    ...
                    <ul className="space-y-2 max-h-40 overflow-y-auto pr-1">
                      {archivos.map((file, index) => (
                        <li
                          key={index}
                          className="flex items-center justify-between bg-gray-100 p-2 rounded-md border border-gray-200"
                        >
