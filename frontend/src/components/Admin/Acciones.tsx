...
import { puedeRevisarTarea, puedeEditarTarea } from "../../utils/tareasExternas";
...
const Acciones: React.FC<AccionesProps> = ({
  ...
  const puedeGestionarEstado = puedeRevisarTarea(tarea, user);
  const puedeEditar = puedeEditarTarea(tarea, user);
  ...
  return (
    <div className="flex items-center justify-center gap-3 h-7 w-full">
      ...
      {tarea.estatus === "PENDIENTE" && (
        <>
          {/* ENTREGAR / CORREGIR (Si el usuario es Responsable de la tarea) */}
          ...
          {puedeGestionarEstado && onCompletar && (
            <div className="group relative flex items-center justify-center">
              <button
                onClick={onCompletar}
